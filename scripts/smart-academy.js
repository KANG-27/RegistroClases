...
async function intentarProgramarClase(page) {
    ...
    for (let intento = 1; intento <= maxIntentos; intento++) {
        ...
        // ... después de elegir fila/horario
        if (DRY_RUN) {
            fs.mkdirSync('screenshots', { recursive: true });
            await page.screenshot({ path: `screenshots/dryrun_${Date.now()}.png`, fullPage: true });
            log('🧪 DRY_RUN activo: NO se hace click final. Navegación OK.');
            return true;
        }
        await frame2.click('#BUTTON1');   // solo si no es dry run
